...
- 譜面の下が **リズムのずれ**です。既定で出ますが、「楽譜にずれを重ねる」で消せます
  ...
  - **1音が1本のバー**です。**左端＝打鍵、右端＝離鍵**。横軸は時間なので、バーの長さがそのまま**音の長さ**、太さが**強さ**になります
  - 縦の目盛りがあるべき打鍵位置（音符の真下）。そこからバー左端までの細い髭が**タイミングのずれ**で、**右へ伸びていれば遅れ、左へ伸びていれば先走り**
  - **目盛りの間隔がそのまま1打点ぶんの時間**です（画面にms数が出ます）
  - 上下の段でバーの位置を見比べると、**左右のズレ**がそのまま分かります
  - この段とベロシティの段は目盛りが別なので、間に区切り線を入れてあります
  - 左端の空き（音部記号と調号のぶんレーンが右に寄ってできる場所）に、**「ずれを重ねる」「調の音に印」の切り替え**と、**メトロノームの振り子**を置いてあります。演奏中に上の設定まで戻らずに済みます
...
**演奏中に文字は出しません。** 打点数や残り拍などを出すと、出た瞬間にその高さぶん下がずれるためです。

#### 薄い「的」と、外れたときの✕

弾く前から、**あるべき姿が薄い枠で出ています**。指定を足すほど的が育ちます。

```
① 指定なし        ·         灰色の小さな丸。打鍵位置だけの的
                 ┊
② 長さを指定      ▭▭▭▭      右へ伸びて「あるべき長さ」になる
                 ┊
③ 強さも指定     ▓▓▓▓▓     厚みがつく。緑＝この太さ以上（強）／橙＝この太さ以下（弱）
                 ┊
   実際に弾いた   ┊─████     ← 的に重ねて描く
```

**外れたときだけ、バーの端の丸が✕に変わります。**

- **左端の✕＝強さが外れた**（押した瞬間に決まるので左端）
- **右端の✕＝長さが外れた**（離して初めて決まるので右端）

色は手の色のままで、形だけが変わります（赤は左手の色と紛らわしいので使いません）。うまく弾けているあいだは✕が出ないので、画面が静かなままです。
...
- **音の長さ**（スタッカート／レガート）… **同じ形の表をもう1つ**。強さと組み合わせられる（強くて短い、など）ので独立しています
  ...
  **ゲート率＝(離鍵−打鍵)÷1打点の長さ**。既定は**長い ≥ 90%／短い ≤ 50%**です。100%は「つぎの音が始まるところまで保持」で、レーンでは**バーの長さが目盛り1つぶん**にあたります。閾値もアクセントと同じで、使う側だけ出ます
...
- **レガート合格のバーは、隣の音の的に重なります。**仕様です（100%＝つぎの音が始まるところなので、それ以上ならはみ出します）
