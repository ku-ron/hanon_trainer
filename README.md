...
**どちらを弾いているかは枠で示します。** 弾いている側の小節を青い枠で囲み、地色をわずかに変えてあります。囲うのは**音符の場所だけ**で、左端の音部記号と調号は含めません（含めると左右の小節で幅も意味も揃わないためです）。演奏中は譜面から目を離せないので、読ませる文字ではなく視野の端で分かる形にしてあります（そのうえで、いま弾く音そのものは青い帯が指しています）。
...
| 長さ | ハノンは**細胞の数**（既定14＝原典と同じ長さ）、音階類は**オクターブ数**（既定2。上行2オクターブ＋下行2オクターブ）。同じ入力欄ですが、値は種類ごとに別々に覚えます |
