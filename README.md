...
**横幅は画面に合わせて組み直します。** 譜面の大きさ（五線の間隔）は画面幅によらず一定で、狭い画面では音符の間だけが詰まります。iPad mini の縦画面（744px）でも横スクロールは出ません。
...
- 譜面の下の細い帯が **リズムのずれ**です。音符と同じ横位置に点が出るので、どの音でよれたかがそのまま分かります。右手と左手を縦線でつないであり、線の長さが左右のズレです。既定で出ますが、「楽譜にずれを重ねる」で消せます
